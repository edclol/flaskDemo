...
BASE_URL = 'http://172.16.2.116:10080'
''''''
...
def job_status(job_name):
    ...
    print(msg)
    if msg['msg'] == "Job not exist!":
        print(msg)
    elif len(msg['tasks']) > 0:
        for task in msg['tasks']:
            print("{} ({}) is/was running on {}".format(task['hostname'], task['status'], task['node']))


def job_stop(job_id):
    # todo: python的表单提交 id=test-159471076460801315119995
    data = {
        'id': (None, job_id)
    }
    print("job_stop----------------")
    sess.headers.update()
    r = sess.post(BASE_URL + '/service?action=job_stop', files=data)
    print(r.status_code)
    print(r.request.url)
...
if __name__ == '__main__':
    ...
    job_list()
    job_stop('test-159471298004725633885138')
